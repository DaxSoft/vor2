...
export function UploadQueue({
  ...
  const currentPath = tasks[0]?.objectKey.includes("/")
    ? `/${tasks[0].objectKey.split("/").slice(0, -1).join("/")}`
    : "/";
    ...
    <section className="glass-panel flex h-full min-h-0 flex-col rounded-panel border border-app-border/20 p-3">
      <div className="mb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            type="button"
            className="rounded-md border border-app-border/20 bg-white/[0.04] p-1 text-xs text-app-muted hover:text-app-text"
